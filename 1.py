...
"""1.读取动态链接库"""
# 依赖的DLL文件(存放在根目录下)
CAN_DLL_PATH = "C:/installitions/workplace/atxcan/ControlCAN.dll"
# 读取DLL文件
...
# 发送数据
# return: 1=OK 0=ERROR
def transmit(VCI_USB_CAN_2, DEV_INDEX, can_index, start_id, end_id, data, interval):
    TRANSMIT_DATA_LEN = len(data)
    if TRANSMIT_DATA_LEN > 8:
        print('Error: 数据长度超过8字节')
        return

    ubyte_array_8 = c_ubyte * 8
    DATA = ubyte_array_8(*data, *(RESERVED for _ in range(8 - TRANSMIT_DATA_LEN)))
    ...
    for can_id in range(start_id, end_id + 1):
        ...
        if ret != STATUS_OK:
            print(f'VCI_Transmit: 通道 {can_index + 1} 发送数据失败，ID: {can_id}')
        
        time.sleep(interval)  # 根据传入的时间间隔进行延时




def receive(VCI_USB_CAN_2, DEV_INDEX, can_index, receive_rate):
    ubyte_array_8 = c_ubyte * 8
    ...
    while True:
        ...
        if ret == STATUS_OK:
            data_hex = ' '.join([hex(byte)[2:] for byte in can_obj.Data])  # 去掉0x前缀
            print(f'ID: {can_obj.ID}  DataLen: {can_obj.DataLen}  Data: {data_hex}')
        else:
             print('VCI_Receive: 通道 ' + str(can_index + 1) + ' 接收数据失败, 正在重试')
        time.sleep(receive_rate)  # 等待指定的接收频率   
...
if __name__ == '__main__':
    ...
    data_to_send = [0x23, 0x2f, 0xff]
    start_id = 100
    end_id = 100
    #receive(VCI_USB_CAN_2, DEV_INDEX, CAN_INDEX_2,0.1)
    #transmit(VCI_USB_CAN_2, DEV_INDEX, CAN_INDEX_1, start_id, end_id, data_to_send,0.1)
    transmit_thread = threading.Thread(target=transmit, args=(VCI_USB_CAN_2, DEV_INDEX, CAN_INDEX_1, start_id, end_id, data_to_send, 0.016))
    receive_thread = threading.Thread(target=receive, args=(VCI_USB_CAN_2, DEV_INDEX,CAN_INDEX_2 ,0.016))
    ...
    """
    # CAN2接收数据
    receive(VCI_USB_CAN_2, DEV_INDEX, CAN_INDEX_1,0.2)
    # 关闭设备
